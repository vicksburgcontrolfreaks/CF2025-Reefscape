...
public class NewCoralArmSubsystem extends SubsystemBase {
   ...
   public void setArmAngle(double targetAngle) {
      ...
      if (ca_integral > ArmConstants.CA_I_MAX)
         ca_integral = ArmConstants.CA_I_MAX;
      if (ca_integral < -ArmConstants.CA_I_MAX)
         ca_integral = -ArmConstants.CA_I_MAX;
   ...
   public void moveArm(double targetExtension) {
      ...
      // we really only want it to work to get remove the last
      // little chunk of error
      if (ce_integral > ArmConstants.CE_I_MAX)
         ce_integral = ArmConstants.CE_I_MAX;
      if (ce_integral < -ArmConstants.CE_I_MAX)
         ce_integral = -ArmConstants.CE_I_MAX;
    ...
    * If lowering the arm (speed < 0) would put the current extension out of its
    * safe zone
   ...
   public void manualAdjustArmAngle(double speed) {
      ...
      // For lowering the arm (speed < 0), if the current extension is more extended
      // than allowed, block the command.
   ...
   /*******************************************************
    * Manual adjustment for arm extension with soft stops that depend on the
    * current angle.
    * Assumes that the extension encoder decreases (becomes more negative) as the
    * arm extends.
    * 
    * @param speed A value that directly commands the motor.
    ******************************************************/
   public void manualAdjustArmExtension(double speed) {
      ...
      // Compute the maximum allowed extension (a negative number) for the current
      // angle.
    ...
    * This method uses your preset target extension values (for low, mid, and high
    * positions),
    * applies a 10% buffer.
    *
   ...
   public double getMaxAllowedExtension(double currentAngle) {
      ...
      if (useLinearLimitLogic) {
         ...
         if (currentAngle <= ArmConstants.lowTgtAngle) {
            ...
            double t = (currentAngle - ArmConstants.lowTgtAngle)
                  / (ArmConstants.midTgtAngle - ArmConstants.lowTgtAngle);
            return lowLimit + t * (midLimit - lowLimit);
         } else if (currentAngle <= ArmConstants.highTgtAngle) {
            double t = (currentAngle - ArmConstants.midTgtAngle)
                  / (ArmConstants.highTgtAngle - ArmConstants.midTgtAngle);
